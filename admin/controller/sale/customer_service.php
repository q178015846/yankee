<?php
class ControllerSaleCustomerService extends Controller {

	public function index()
	{
		$this->load->language('sale/customer_service');
		$this->load->model('sale/customer_service');
		$this->document->setTitle("多客服管理");
		$this->getList();
	}
	...
	protected function getList() {
		...
		$user_total = $this->model_sale_customer_service->getTotalUsers();

		$results = $this->model_sale_customer_service->getUsers($filter_data);
		...
		foreach ($results as $result) {
			$data['users'][] = array(
				...
				'edit'       => $this->url->link('sale/customer_service/edit', 'token=' . $this->session->data['token'] . '&user_id=' . $result['user_id'] . $url, 'SSL')
			);
		}
